...
void AverageColumns(int[,] array3)
{
    Console.WriteLine($"Average values of these columns are:");
    double[] averCol = new double[array3.GetLength(1)];
    for (int j = 0; j < array3.GetLength(1); j++)
    {
        for (int i = 0; i < array3.GetLength(0) ; i++)
        {
            averCol[j] += array3[i, j];
            //Console.Write($"{array3[i, j]}  ");
        }
        averCol[j] = averCol[j] / array3.GetLength(0);
        Console.Write($"{averCol[j]:N2}  ");
